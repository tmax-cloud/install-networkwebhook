...
usage() {
    cat <<EOF
Generate certificate suitable for use with an network-hook service.
This script uses k8s' CertificateSigningRequest API to a generate a
certificate signed by k8s CA suitable for use with network-hook
...
The following flags are required.
       --service          Service name of network-hook.
       --namespace        Namespace where network-hook service and secret reside.
...
openssl req -new -key ${tmpdir}/server-key.pem -subj "/CN=system:node:master;/O=system:nodes" -out ${tmpdir}/server.csr -config ${tmpdir}/csr.conf
